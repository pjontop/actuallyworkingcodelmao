...
///
// Blue Left Autonomous, 45, Straight, Left -> LG, w/ descore
///
void BlueLeftAuto() {
...
void blueleftACTUALauton() {

    // first part

    chassis.pid_odom_set({{0_in, -30_in}, rev, 127}); // go to the three balls near the centre-corner
    chassis.pid_wait_until(-20_in); // deploy pneumatics at the brink of the first ball to "trap" it and intake
    Intake.move(-127); // intake the balls
    dropper.set(true); // deploy pneumatics
    chassis.pid_wait();
...
///
// Blue Left New Auton (ZCode)
///
void blueleftnewauton() {
  chassis.pid_odom_set({{0_in, -30_in}, rev, 110});
  ...
  dropper.set(true);
    chassis.pid_wait();
        ...
        // {{37_in, -12.35_in}, rev, 110},  // Fixed: added closing brace for point


  chassis.pid_odom_set({
        //{{17_in, -19_in}, rev, 110}, 
        {{44.21_in, -3.4_in, -156_deg}, rev, 110} // Fixed: added opening brace and theta
    }, false); //67_in, 110, true)
  ...
  chassis.pid_odom_set({{32.33_in, -25.75_in, -156_deg}, rev, 110}, true);
  chassis.pid_wait();
